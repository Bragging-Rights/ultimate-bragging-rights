//this the previous user controller which is not in use now
...
exports.signUpController = async (req, res) => {
  ...
  db.query("SELECT * FROM users WHERE email = ?", [email], (error, user) => {
    if (user && user?.length > 0) {
      ...
      return res
        .status(400)
        .json(responseObject({}, "An unknown error occured", true));
    }
  });
};
  ...
  db.query(query, [email], (error, results) => {
    if (error) {
      ...
      if (results.length > 0) {
        ...
        if (findUser) {
          ...
          if (checkPassword) {
            ...
            jwt.sign(payload, config.jwtSecret, (err, token) => {
              if (err)
                ...
                res
                  .status(200)
                  .json(
                    responseObject(findUser, "Logged In Successfully", false)
                  );
              }
            });
          } else {
            res
              .status(201)
              .json(
                responseObject({}, "Incorrect username or password.", true)
              );
          }
        } else {
          res
        ...
        res
          .status(404)
          .send(responseObject({}, "Incorrect username or password.", false)); // No matching row found
      }
    }
  });
};
...
exports.verifyOTP = async (req, res) => {
  ...
  db.query(query, [email], (error, results) => {
    if (error) {
      ...
      if (otp === user?.otp) {
        db.query(
          "UPDATE users SET `otp` = ?, `emailVerified` = ?",
          ["", true],
          (err, result) => {
            if (err) {
              console.log(err);
